...
import { apiService, type Unit } from './services/api';
import './App.css';
...
function App() {
  ...
  // Baue Baumstruktur aus flacher Liste
  const buildTree = (units: Unit[]): TreeNode[] => {
    const unitMap = new Map<number, TreeNode>();
    const roots: TreeNode[] = [];

    // Erstelle Map mit allen Einheiten
    units.forEach(unit => {
      unitMap.set(unit.id, { ...unit, children: [] });
    });

    // Füge Kinder zu Eltern hinzu
    units.forEach(unit => {
  ...
  // Positioniere die Nodes für das Organigramm
  const positionNodes = (nodes: TreeNode[], startY: number = 100, level: number = 0) => {
    ...
    const drawConnections = (nodes: TreeNode[]) => {
      nodes.forEach(node => {
        node.children.forEach(child => {
          if (node.x && node.y && node.width && node.height && child.x && child.y) {
            connections.push(
              <line
  ...
  const getNodeColor = (level: number): string => {
    const colors = [
      '#FFE5E5', // Level 0 - Rot
      '#FFF5E6', // Level 1 - Orange
      '#E8F5E8', // Level 2 - Grün
      '#E8F4FD', // Level 3 - Blau
      '#F4ECF7', // Level 4 - Lila
      '#FFF0F5', // Level 5 - Rosa
      '#F0FFF0'  // Level 6 - Hellgrün
    ];
  ...
  const exportToPNG = async () => {
    ...
    try {
      ...
      link.download = `organigramm-esercito-italiano-${new Date().toISOString().split('T')[0]}.png`;
      link.href = canvas.toDataURL();
      link.click();
    } catch (err) {
      console.error('Fehler beim PNG-Export:', err);
      alert('PNG-Export fehlgeschlagen. Bitte html2canvas installieren: npm install html2canvas');
    }
  };
  ...
  const exportToSVG = () => {
    ...
    const link = document.createElement('a');
    link.download = `organigramm-esercito-italiano-${new Date().toISOString().split('T')[0]}.svg`;
    link.href = url;
    link.click();
      ...
      <header className="App-header">
        <div className="header-content">
          <div className="title-section">
            ...
            <p>Erweitertes Organigramm - Komplette Hierarchiestruktur</p>
          </div>
      ...
      <main className="main-content">
        <div className="organigramm" ref={organigramRef}>
          <div className="org-title">
            <h2>Vollständige Organisationsstruktur des Italienischen Heeres</h2>
            <div className="org-stats">
              <span>Einheiten: {organisation.length}</span>
              <span>Personal: {organisation.reduce((sum, unit) => sum + (unit.personnel_count || 0), 0)}</span>
            </div>
          </div>
          ...
          <div className="svg-container">
            ...
            >
              ...
              {/* Verbindungslinien zuerst zeichnen (im Hintergrund) */}
              {renderConnections()}
              
              {/* Dann die Nodes (im Vordergrund) */}
